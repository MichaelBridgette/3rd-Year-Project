...
EndLevelScreen::EndLevelScreen(std::string levelName, Game & game, sf::RenderWindow & window) :
	m_game(&game),
	m_window{window}
{
	transparency = 0;
	...
	m_text.setString("Level Complete!");
...
void EndLevelScreen::update()
{
	if (transparency < 255)
	{
		...
		m_text.setPosition(m_window.getView().getCenter() + sf::Vector2f(-350, -150));
